        ...
        content
            .onChange(of: scenePhase) { _, newPhase in
                if newPhase == .background || newPhase == .inactive {
                    // Synchronous: saveOnBackground() must take the UIKit background-task
                    // assertion now, while the app is still running — before it spawns the
                    // async save. Wrapping this in a Task would defer the assertion past a
                    // possible suspension point.
                    coordinator.saveOnBackground()
                }
            }
    }
}
